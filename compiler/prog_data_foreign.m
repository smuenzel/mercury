...
:- func init_foreign_import_modules = c_j_cs_ml_fims.

:- pred add_fim(foreign_language::in, module_name::in,
    c_j_cs_ml_fims::in, c_j_cs_ml_fims::out) is det.
...
:- func get_all_fim_specs(c_j_cs_ml_fims) = set(fim_spec).

:- func get_all_foreign_import_modules(c_j_cs_ml_fims) = set(module_name).

:- func get_lang_fim_specs(c_j_cs_ml_fims, foreign_language) = set(fim_spec).

:- func get_lang_fim_modules(c_j_cs_ml_fims, foreign_language)
    = set(module_name).

:- implementation.

init_foreign_import_modules =
    c_j_cs_ml_fims(set.init, set.init, set.init, set.init).
...
add_fim(Lang, ModuleName, !FIM) :-
    ...
    ;
        Lang = lang_ocaml,
        ModuleNames0 = !.FIM ^ fim_ocaml,
        ( if set.insert_new(ModuleName, ModuleNames0, ModuleNames) then
            !FIM ^ fim_ocaml := ModuleNames
        else
            true
        )
    ).
...
get_all_fim_specs(FIM) = FIMSpecs :-
    FIM = c_j_cs_ml_fims(ModuleNamesC, ModuleNamesJava, ModuleNamesCSharp,
        ModuleNamesOcaml),
    FIMSpecs = set.union_list([
        ...
        set.map(make_fim_spec(lang_ocaml), ModuleNamesOcaml)
        ]).

get_all_foreign_import_modules(FIM) = ModuleNames :-
    FIM = c_j_cs_ml_fims(ModuleNamesC, ModuleNamesJava,
        ModuleNamesCSharp, ModuleNamesOcaml),
